...
while True:
    start = perf_counter()
    filename = fileStart+str(chartDate.year)+"-"+("0" if len(str(chartDate.month)) == 1 else "")+str(chartDate.month)+fileEnd
    if os.path.isfile(filename):
        chartDate += timedelta(weeks=1)
        continue
    chart = bb.ChartData("hot-100", str(chartDate), timeout=6, max_retries=2)
    chartDict = {
